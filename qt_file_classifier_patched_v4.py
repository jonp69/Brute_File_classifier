...
Added statistics tracking and improved directory filtering.
"""
...
# Dictionary to track file types and counts
file_type_stats = {}
last_stats_time = 0
STATS_INTERVAL = 5  # Show stats every 5 seconds

# Variables for time estimation and scan progress
scan_start_time = 0
...
# Try to find the correct path for imports
try:
    ...
    from file_classifier import (
        load_config, save_config, load_database,
    ...
    # Import the file database - use a copy to avoid modifications during import
    file_database = dict(file_classifier.file_database)
    ...
    def on_scan(self):
        ...
        if dialog.exec_():
            ...
            if selected_drives:
                ...
                if resume_state:
                    # Check if selected drives match the resume state
                    if set(selected_drives) == set(resume_state["drive_paths"]):
                        self.scanner_thread = ScannerThread(selected_drives, resume_state["scan_mode"])
                        self.scanner_thread.processed_files_count = resume_state["processed_files_count"]
                        self.scanner_thread.total_files_count = resume_state["total_files_count"]
                        self.scanner_thread.is_cancelled = False  # Ensure it's not cancelled
                        
                        self.task_label.setText(f"Resuming {mode_desc} scan...")
                    else:
                        # Different drives selected, start new scan
                        self.scanner_thread = ScannerThread(selected_drives, self.selected_mode)
                        self.task_label.setText(f"Starting new {mode_desc} scan...")
                else:
                    # No resume state, start fresh scan
                    self.scanner_thread = ScannerThread(selected_drives, self.selected_mode)
                    self.task_label.setText(f"Starting new {mode_desc} scan...")
...
def start_app():
    """Start the application - entry point for the script"""
